...
function SectionLabel({ children }: { children: string }) {
  return (
    <h2 className="mb-3 flex items-center gap-3 font-sans text-xs font-bold uppercase tracking-[0.22em] text-accent">
      <span>{children}</span>
...
export function ResumeSheet() {
  return (
    <div className="space-y-8 sm:space-y-10 print:space-y-0">
      {/* PAGE 1 */}
      <article className="resume-page mx-auto flex flex-col justify-between bg-cream px-7 py-7 text-ink shadow-[0_24px_60px_-24px_rgba(26,35,50,0.35)] sm:px-10 sm:py-9">
        <div>
          {/* Header */}
          <header className="border-b border-rule pb-4">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-y-1.5 gap-x-4 border-t border-rule/50 pt-2.5 font-sans text-xs sm:text-[13px] text-ink-soft">
              <div>
          ...
          <div className="mt-6 space-y-6 sm:space-y-7">
            ...
            <section>
              ...
              <p className="text-[13.5px] sm:text-sm leading-relaxed text-ink-soft">{resume.summary}</p>
            </section>
              ...
              <div className="space-y-5">
                {resume.experience.map((job) => (
                  <div key={job.org} className="rounded-sm">
                    ...
                    <p className="mt-0.5 text-xs sm:text-sm font-semibold text-accent">
                      {job.org}
        ...
        {/* Page 1 Footer */}
        <footer className="mt-6 flex items-center justify-between border-t border-rule/70 pt-3 text-xs text-muted">
      ...
      <article className="resume-page mx-auto flex flex-col justify-between bg-cream px-7 py-7 text-ink shadow-[0_24px_60px_-24px_rgba(26,35,50,0.35)] sm:px-10 sm:py-9">
        <div>
          ...
          <div className="mt-6 space-y-6 sm:space-y-7">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {Object.entries(resume.skills).map(([group, items]) => (
                  <div key={group} className="rounded border border-rule/80 bg-paper/40 p-3.5 sm:p-4 text-xs">
                    ...
                    <ul className="mt-2 list-disc pl-4 space-y-1.5 text-xs sm:text-[12.5px] leading-snug text-ink-soft">
                      {items.map((item) => (
                        <li key={item}>{item}</li>
                      ))}
                    </ul>
            ...
            <section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded border border-rule/80 bg-paper/40 p-3.5 text-xs">
                  <p className="font-bold uppercase tracking-wider text-ink text-xs">Design</p>
                  <ul className="mt-2 list-disc pl-4 space-y-1.5 text-xs sm:text-[12px] leading-snug text-ink-soft">
                ...
                <div className="rounded border border-rule/80 bg-paper/40 p-3.5 text-xs">
                  <p className="font-bold uppercase tracking-wider text-ink text-xs">AI Tools</p>
                  <ul className="mt-2 list-disc pl-4 space-y-1.5 text-xs sm:text-[12px] leading-snug text-ink-soft">
                    <li>Cursor AI</li>
                    <li>Antigravity</li>
                    <li>Claude</li>
                ...
                <div className="rounded border border-rule/80 bg-paper/40 p-3.5 text-xs">
                  <p className="font-bold uppercase tracking-wider text-ink text-xs">Frontend</p>
                  <ul className="mt-2 list-disc pl-4 space-y-1.5 text-xs sm:text-[12px] leading-snug text-ink-soft">
                    <li>HTML5 & CSS3</li>
                    <li>Tailwind CSS</li>
                    <li>Wix Studio</li>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3.5 sm:grid-cols-2">
                  {resume.education.map((ed) => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-ink-soft font-medium">{ed.credential}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {resume.certifications.map((c) => (
                    ...
                    >
                      <p className="font-semibold text-ink leading-snug">{c.name}</p>
                      <p className="mt-1.5 text-[10.5px] font-medium uppercase tracking-wider text-accent">
                        {c.org}
                      </p>
                    </div>
                  ))}
            ...
            {/* Languages */}
            <section className="flex flex-wrap items-center justify-between gap-2 border-t border-rule/80 pt-3.5 text-xs text-ink-soft">
              <span className="font-bold uppercase tracking-wider text-accent">Languages</span>
              <span className="font-medium text-ink text-[13px]">{resume.languages.join("   ·   ")}</span>
            </section>
          </div>
        </div>

...
function Heading({ children }: { children: string }) {
  return (
    <h2 className="mb-2 border-b-2 border-ink font-ats text-[13.5px] sm:text-[14px] font-bold uppercase tracking-wider text-ink pb-0.5">
      {children}
    </h2>
  );
}
...
export function AtsResume() {
  ...
  return (
    <div className="space-y-8 sm:space-y-10 print:space-y-0 font-ats">
      {/* PAGE 1 */}
      <article className="resume-page mx-auto flex flex-col justify-between bg-white px-6 py-6 sm:px-9 sm:py-7 text-ink shadow-[0_24px_60px_-24px_rgba(26,35,50,0.28)]">
        <div>
          {/* Header */}
          <header className="text-left border-b-2 border-ink pb-3">
            ...
            <p className="font-ats text-[14.5px] sm:text-[15px] font-bold text-ink mt-0.5">{resume.role}</p>
            <p className="mt-1.5 font-ats text-[12.5px] sm:text-[13px] leading-normal text-ink">
              {resume.phone} &nbsp;|&nbsp; {resume.email} &nbsp;|&nbsp; {resume.location}
            </p>
            <p className="mt-0.5 font-ats text-[12.5px] sm:text-[13px] text-ink">
          ...
          <section className="mt-3.5 sm:mt-4">
            ...
            <p className="font-ats text-[12.5px] sm:text-[13px] leading-relaxed text-ink">{resume.summary}</p>
          </section>
              ...
              {resume.experience.map((job) => (
                <div key={job.org}>
                  <div className="flex flex-wrap items-baseline justify-between">
                    <p className="font-ats text-[14px] sm:text-[14.5px] font-bold text-ink">{job.title}</p>
                    <span className="font-ats text-[11.5px] sm:text-[12px] font-semibold text-ink">{job.dates.replace("—", "-")}</span>
                  </div>
                  <p className="font-ats text-[12px] sm:text-[12.5px] font-semibold text-ink mt-0.5">
                    {job.org} &nbsp;|&nbsp; {job.type}
                  </p>
                  <ul className="mt-1.5 list-disc pl-4 font-ats text-[12px] sm:text-[12.5px] leading-snug text-ink space-y-1">
                    {job.bullets.map((b) => (
                      <li key={b}>{b}</li>
                    ))}
                  </ul>
          ...
          <section className="mt-3.5 sm:mt-4">
            ...
            <div className="space-y-2">
              {resume.projects.map((p) => (
                <div key={p.title} className="font-ats text-[12px] sm:text-[12.5px] leading-snug text-ink">
        ...
        {/* Page 1 Footer */}
        <footer className="mt-3 flex items-center justify-between border-t border-ink/40 pt-2 text-[11px] sm:text-[11.5px] text-ink">
      ...
      <article className="resume-page mx-auto flex flex-col justify-between bg-white px-6 py-6 sm:px-9 sm:py-7 text-ink shadow-[0_24px_60px_-24px_rgba(26,35,50,0.28)]">
        <div>
          ...
          <section>
            ...
            <ul className="list-disc pl-4 font-ats text-[12px] sm:text-[12.5px] leading-snug text-ink space-y-1.5">
              <li>
          ...
          <section className="mt-3.5 sm:mt-4">
            ...
            <p className="font-ats text-[12px] sm:text-[12.5px] leading-relaxed text-ink">
          ...
          <section className="mt-3.5 sm:mt-4">
            ...
            <div className="space-y-2.5 font-ats text-[12px] sm:text-[12.5px] leading-snug text-ink">
